...
class AddUsers:
    ...
    def __init__(self, button3):
        ...
        self.vbox1.show()
        title = Gtk.Label(label=get_text("User Setup"), name="Header")
        ...
        self.vbox1.pack_start(box2, False, False, 0)
        box2.show()
        label = Gtk.Label(label='<b>' + get_text('User Account') + '</b>')
        label.set_use_markup(True)
        label.set_alignment(.2, .2)
        Username = Gtk.Label(label=get_text("User name"))
        self.user = Gtk.Entry()
        self.label2 = Gtk.Label(label=get_text("Real name"))
        self.name = Gtk.Entry()
        self.name.connect("changed", self.user_and_host)
        self.labelpass = Gtk.Label(label=get_text("Password"))
        self.password = Gtk.Entry()
        self.password.set_visibility(False)
        self.password.connect("changed", self.password_verification, button3)
        self.label4 = Gtk.Label(label=get_text("Verify Password"))
        ...
        self.label5 = Gtk.Label(label=get_text("Shell"))
        ...
        label = Gtk.Label(label='<b>' + get_text('Set Hostname') + '</b>')
        label.set_use_markup(True)
        label.set_alignment(0, .5)
        table = Gtk.Table(1, 3, True)
        table.set_row_spacings(10)
        pcname = Gtk.Label(label=get_text("Hostname"))
        self.host = Gtk.Entry()
